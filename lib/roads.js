// Procedural ground: road surface and markings, and lawn.
//
...
// running along v, i.e. an ns road) or 'v'; stall: an edge to draw a parking
// stall line on.
function paint(img, { edges = {}, crosswalk = null, stall = null } = {}) {
  for (let y = 0; y < tileH; y++) for (let x = 0; x < tileW; x++) {
    ...
    if (stall) {
      const d = { u0: u, u1: 1 - u, v0: v, v1: 1 - v }[stall];
      const t = stall[0] === 'u' ? v : u;
      if (d < 1.5 * PX && t < 0.72) set(img, x, y, C.white);
    }
...
function roadSet() {
  ...
  for (const axis of ['ns', 'ew']) {
    ...
    tiles.push({ id: `ground_parking_${axis}`, stall: axis === 'ns' ? 'v0' : 'u0' });
  }
...
module.exports = { roadSet, render, asphalt, paint, grass };
